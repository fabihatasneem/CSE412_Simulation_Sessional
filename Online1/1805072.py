...
# File handling
infile = open("IOs - OnlineB/io1/in.txt", "r")
...
end_time = 0.0
queue_size = 0
time_arrival = [0.0] * (int(queue_size) + 1)
time_last_event = 0.0
time_next_event = [0.0] * 4
total_of_delays = 0.0
event_count = 0
customer_count = 0
balking_percentage = 0.0

# Initialize function
def initialize():
    global simulation_time, time_arrival, server_status, num_in_q, time_last_event, num_customers_delayed, total_of_delays, area_num_in_q, area_server_status, queue_size
    simulation_time = 0
    server_status = IDLE
    num_in_q = 0
    time_arrival = [0.0] * (int(queue_size) + 1)
...
def arrive():
    global simulation_time, server_status, num_in_q, total_of_delays, num_customers_delayed, event_count, customer_count, queue_size, time_arrival
    time_next_event[1] = simulation_time + expon(mean_interarrival)
    event_count += 1
    customer_count += 1
...
def report():
    global total_of_delays, num_customers_delayed, area_num_in_q, simulation_time, area_server_status, balking_percentage, customer_count
    ...
    resultfile.write(f"\nTotal number of Balking customers: {int(customer_count - num_customers_delayed)}\n")
    resultfile.write(f"Total number of customers delayed: {int(num_customers_delayed)}\n")
    
    balking_percentage = ((customer_count - num_customers_delayed) * 100 ) / customer_count
    resultfile.write(f"Percentage of Balking customers: {balking_percentage:.2f}%\n")
...
def expon(mean):
    return -mean * math.log(lcgrand(4))

# Main simulation loop
mean_interarrival, mean_service, end_time, queue_size = map(float, infile.readline().split())
initialize()
while True:
    timing()
